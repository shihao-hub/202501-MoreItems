...
local icepack_auto_open = TUNING.MONE_TUNING.GET_MOD_CONFIG_DATA.icepack_auto_open;
...
local function auto(inst, pickupguy)
    if inst.components.container and pickupguy then
        ...
        local find_icepack = false;
        local find_spicepack = false;
        for i = 1, numslots do
            ...
            if icepack_auto_open and not find_icepack and item and item.prefab == "mone_icepack" and item.components.container then
                find_icepack = true;
                item.components.container:Open(pickupguy);
            end
            if spicepack_auto_open and not find_spicepack and item and item.prefab == "mone_spicepack" and item.components.container then
                find_spicepack = true;
                item.components.container:Open(pickupguy);
            end
        end
...
local function icepack_auto_open__onitemget(inst, data)
    local item = data and data.item;
    if item == nil or item.prefab ~= "mone_icepack" then
        return
    end
    if not item.components.container:IsOpen() then
        local owner = inst.components.inventoryitem:GetGrandOwner();
...
local function spicepack_auto_open__onitemget(inst, data)
    local item = data and data.item;
    if item == nil or item.prefab ~= "mone_spicepack" then
        return
    end
    if not item.components.container:IsOpen() then
        local owner = inst.components.inventoryitem:GetGrandOwner();
...
local function fn()
    ...
    -- 25/05/31 我真服了，难怪我一直不加这个内容，这可拓展性为 0
    if icepack_auto_open then
        inst:ListenForEvent("itemget", icepack_auto_open__onitemget)
    end    
    if spicepack_auto_open then
        inst:ListenForEvent("itemget", spicepack_auto_open__onitemget)
    end
