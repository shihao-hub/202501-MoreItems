    ...
    if data.amount and data.amount < 0 then
        ...
        if canBackWound(afflicter) then
            if afflicter.components.health then
                ...
                -- 2025/05/31: 添加月后青蛙（明眼青蛙）
                if afflicter.prefab == "frog" 
                    or afflicter.prefab == "lunarfrog"
                    or afflicter.prefab == "spiderqueen" then
                    DAMAGE = afflicter.components.health.maxhealth;
                end
...
local function fn()
    ...
    inst.components.health:SetMaxHealth(3000); -- 2025/05/31: 加到 3000 吧，扛得住两个蜘蛛巢比较好
    inst.components.health:StartRegen(TUNING.BEEFALO_HEALTH_REGEN * 3, TUNING.BEEFALO_HEALTH_REGEN_PERIOD / 4); -- 牛10秒回一次
    ...
    inst.taunt_task = inst:DoPeriodicTask(2, function(inst)
        ...
        local CANT_TAGS = {
            ...
            "groundspike", "smashable", "companion", "abigail", "shadowminion",
            "mone_dummytarget", "bird", "mone_beef_bell_beefalo"
        };
