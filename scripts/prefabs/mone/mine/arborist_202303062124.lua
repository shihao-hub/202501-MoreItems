...
local function CheckFire(inst)
    ...
    if x and y and z then
        local MUST_TAGS = nil;
        local CANT_TAGS = { "FX", "INLIMBO", "NOCLICK" };
        local ONEOF_TAGS = { "smolder", "fire" };

        local ents = TheSim:FindEntities(x, y, z, find_range, MUST_TAGS, CANT_TAGS, ONEOF_TAGS);
...
local function PlantTrees(inst)
    ...
    --检索范围内的物体，灭火
    if TUNING.MONE_TUNING.GET_MOD_CONFIG_DATA.arborist_fire_extinction then
        CheckFire(inst)
    end
