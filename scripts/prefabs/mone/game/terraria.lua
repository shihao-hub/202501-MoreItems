...
local function MakeHat(name)
    ...
    local function simple(custom_init)
        ...
        inst._old_planardefense_value = 20
        inst.components.planardefense:SetBaseDefense(inst._old_planardefense_value)
    ...
    local function onpercentusedchange(inst, data)
        ...
        if percent <= 0 then
            ...
            if planardefense then
                planardefense:SetBaseDefense(0)
            end
        else
            armor.absorb_percent = ABSORB_PERCENT;
            if planardefense then
                planardefense:SetBaseDefense(inst._old_planardefense_value)
            end
        end
    end
    ...
    fns.eyemask = function()
        ...
        inst.components.armor:InitCondition(TUNING.ARMORRUINS, ABSORB_PERCENT)
...
local function MakeArmor()
    ...
    local function onequip(inst, owner)
        ...
        if skin_build ~= nil then
            ...
            owner.AnimState:OverrideSymbol("swap_shield", "swap_eye_shield", "swap_shield")
        end
    ...
    local function onpercentusedchange(inst, data)
        ...
        if percent <= 0 then
            armor.absorb_percent = 0;
            weapon:SetDamage(17);
            if planardamage and planardefense then
            ...
            if planardamage and planardefense then
                planardamage:SetBaseDamage(inst._old_planardamage_value)
                planardefense:SetBaseDefense(inst._old_planardefense_value)
            end
        end
    end
    ...
    local function fn()
        ...
        inst._old_planardamage_value = 20
        inst.components.planardamage:SetBaseDamage(inst._old_planardamage_value)
