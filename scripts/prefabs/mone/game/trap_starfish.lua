...
local function start_reset_task(inst)
    ...
    local reset_task_randomized_time = GetRandomWithVariance(3, TUNING.STARFISH_TRAP_NOTDAY_RESET.VARIANCE)
...
local function on_deactivate(inst)
    if inst.components.lootdropper ~= nil then
        inst.components.lootdropper:SpawnLootPrefab("mone_dug_trap_starfish")
    end
...
return Prefab("mone_trap_starfish", trap_starfish, assets, prefabs),
    Prefab("mone_dug_trap_starfish", dug_trap_starfish, assets, prefabs),
    MakePlacer("mone_dug_trap_starfish_placer", "star_trap", "star_trap", "trap_idle")
