...
local regular_assets =
{
    ...
    Asset("ANIM", "anim/mone_beef_bell.zip"),
}
...
local function CleanUpBell(inst)
    ...
    inst.components.inventoryitem.imagename = "mone_beef_bell";
    inst.components.inventoryitem.atlasname = "images/inventoryimages/mone_beef_bell.xml";
    inst:PushEvent("imagechange")

    inst.AnimState:PlayAnimation("mone_beef_bell", false)
...
local function OnUsedOnBeefalo(inst, target, user)
    ...
    if successful then
        ...
        inst.components.inventoryitem.imagename = "mone_beef_bell_linked";
        inst.components.inventoryitem.atlasname = "images/inventoryimages/mone_beef_bell_linked.xml";
        inst:PushEvent("imagechange")
        inst.AnimState:PlayAnimation("mone_beef_bell_linked", true)
...
local function CommonFn(data)
    ...
    inst.MiniMapEntity:SetIcon("mone_beef_bell.tex")

    MakeInventoryPhysics(inst)

    inst.AnimState:SetBank("mone_cowbell")
    inst.AnimState:SetBuild("mone_beef_bell")
    inst.AnimState:PlayAnimation("mone_beef_bell", false)

    MakeInventoryFloatable(inst, nil, 0.05, FLOAT_SCALE)

    inst:AddTag("bell")
    inst:AddTag("mone_bell")
...
return Prefab("mone_beef_bell",        RegularFn, regular_assets                )
