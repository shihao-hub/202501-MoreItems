...
function singletons:set(self, k, v)
    if self[k] then
        error("DuplicateKeyError: The singletons table prohibits multiple assignments to the same key (Code 409).")
    end
    self[k] = v
end

function singletons:get(self, k)
    return self[k]
end
...
---
---修复 2025-04-11 发现的，因为饥荒更新导致的皮肤 bug。
---
---@param env env
local function fix_basic_init_fn(env, prefabname, prefabs)
    local key = "ec3fced5-21e7-442f-b85b-19ffdca225bf" .. ":" .. "fn"
    singletons[key] = singletons[key] or function(inst)
        if not TheWorld.ismastersim then
            return inst
        end
        if not inst.OnBackpackSkinChanged then
            inst.OnBackpackSkinChanged = function() end
        end
    end
    if prefabname == "backpack" then
        for _, name in pairs(prefabs) do
            env.AddPrefabPostInit(name, singletons[key])
        end
    end
end
...
---
---@param prefabname string 游戏内对应的那个预制物的代码名
---@param build string 那个对应的预制物的 build
---@param prefabs table[] 有哪些预制物要被修改呢？
function API.reskin(prefabname, build, prefabs, kwargs)
    if kwargs and kwargs.env then
        fix_basic_init_fn(kwargs.env, prefabname, prefabs)
    end
...
function API.reskin2(env, prefabname, bank, build, prefabs)
    fix_basic_init_fn(env, prefabname, prefabs)
...
local FunctionAPI_as_fns = {
    ...
    transferIntoSomeCon = function(self, con, slot)
        ...
        elseif con.prefab == "mone_seedpouch" then
            if item and item:IsValid() and con and con.components.container and con.prefab ~= self.prefab and item.components.inventoryitem.cangoincontainer then
               item = self.components.container:RemoveItemBySlot(slot);
               item.prevslot = nil;
               item.prevcontainer = nil;
...
local function FunctionAPI_isExcludedSomething(inst)
    ...
    local prefabslist = {
        ...
        "mie_ordinary_bundle_state2",
        "phonograph","record", -- 排除留声机和唱片
    };
