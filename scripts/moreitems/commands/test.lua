...
local function CallHelper()
    ...
    local function _is_excluded_directory(directory)
    ...
    local function _is_included_file(file)
        return stl_string.endswith(file, ".lua")
                and not stl_string.endswith(file, "commands.lua")
                and not stl_string.endswith(file, "automation.lua")
    end

    local function list_files(directory)
        if _is_excluded_directory(directory) then
            return {}
        end
        ...
        for file in lfs.dir(directory) do
            --[[
                ...
                > 确实不太好，但是其实还是那句话，性能优化应该是系统开发完成之后进行热点分析的事情。
                >
                > 此处的循环创建函数优化起来也很简单，大不了在 for 上定义一个 local process_file 函数，
                > 然后在循环中调用 process_file，类似 file 这种可变闭包参数可以改为参数传递，很简单！
                >
        ...
        return Stream.of(res)
                     ...
                     :filter(function(e) return _is_included_file(e) end)
                     :totable()
    end
