...
---是服务端
---
function module.is_server_side()
...
---是客户端
---
function module.is_client_side()
    return TheNet:GetIsClient() or TheNet:GetIsServer() and not TheNet:IsDedicated()
end
...
---
---根据 key 获得 TUNING.MONE_TUNING.GET_MOD_CONFIG_DATA 中配置的信息
---
...
local function iife_generate_hook_fn()
    local l_cache = {}
    ...
    local function hook_fn(old_fn, generate_present_fn, unique_key)
        ...
        unique_key = helper.generate_unique_key(old_fn, unique_key)

        if unique_key == nil then
            return generate_present_fn(old_fn)
        end
        if l_cache[unique_key] == nil then
            l_cache[unique_key] = generate_present_fn(old_fn)
            log.info(base.string_format("cache misses, cached data: `{{key}} -> {{value}}`", { key = unique_key, value = l_cache[unique_key] }))
        else
            log.info(base.string_format("cached data: `{{key}} -> {{value}}`", { key = unique_key, value = l_cache[unique_key] }))
        end
        return l_cache[unique_key]
    end
...
--[at risk] 存在风险的函数
module.hook_fn = iife_generate_hook_fn()
