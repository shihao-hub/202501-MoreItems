...
local function simple_async_framework_design()
    local function iife_async_utils()
        local module = {}
    ...
    local async_utils = iife_async_utils()
    ...
    local function scheduler(tasks)
        ...
        return coroutine.create(function()
            ...
            while true do
                ...
                helper.handle_non_blocking_tasks(tasks)

                -- 主线程做一些其他事情
                -- NOTE: 我认为，主线程也需要一个另外的其他事情队列，假设那个线程是 arrange_thing_real_thread
                helper.do_non_blocking_things()
                ...
                -- But Lua does not provide the `sleep` function,
                -- so I use `os.execute` to call the `sleep` command?
