...
local function _increase(self, key, step)
    ...
    -- 可惜 Lua 没有 lambda, () -> value 多好
    value = base.if_then_else(value, function() return value end, function() return 0 end)
...
if select("#", ...) == 0 then
    ...
    -- NOTE: 单元测试很重要！哪怕没有全部覆盖！只要写了就有用！而且很多情况只要覆盖关键的即可。
    xpcall(function()
