...
if IsModEnabled("精灵公主")
        or IsModEnabled("Musha")
        or IsModEnabled("439115156")
        or IsModEnabled("2580566069")
        or IsModEnabled("2269092717")
        or IsModEnabled("2398672953")
        or IsModEnabled("2463027103")
        or IsModEnabled("2820496429")
        or IsModEnabled("2269092717")
        or IsModEnabled("1916128763")
then
    musha = true;
end
...
-- 学到了：
---- FindBestContainer 明明找到的是函数，最后执行的时候居然变成了：attempt to call upvalue 'FindBestContainer' (a string value)
------ debug函数，直接设置本地变量。可能调用的不是你这个函数变量的地址？但是为啥还是不知道啊。不管了！
-------- 因为这个会导致每次初始化都执行一次。。。。。。
...
local old_TradeItem = InvSlot.TradeItem;
local FindBestContainer, up;
if old_TradeItem then
    FindBestContainer, up = GetLocalFn(old_TradeItem, "FindBestContainer");
end
if FindBestContainer and up then
    SetLocalFn(old_TradeItem, up, function(self, item, containers, exclude_containers)
        ...
        if character ~= nil and inventory ~= nil and container_item ~= nil then
            ...
            local backpack;
            local mie_book_silviculture, structure;
            local mone_tool_bag;
            local mone_icepack;
            local mone_spicepack;
            for c, _ in pairs(opencontainers) do
                if c.prefab == "mone_storage_bag" and mone_storage_bag == nil then
                    mone_storage_bag = c;
                end
                if c:HasTag("stewer") and stewer == nil then
                    stewer = c;
                end
                if c.prefab == "mone_backpack" and mone_backpack == nil then
                    mone_backpack = c;
                end
                if c:HasTag("backpack") and backpack == nil then
                    backpack = c;
                end
                if c.prefab == "mone_tool_bag" and mone_tool_bag == nil then
                    mone_tool_bag = c;
                end
                if c.prefab == "mone_spicepack" and mone_spicepack == nil then
                    mone_spicepack = c;
                end                
                if c.prefab == "mone_icepack" and mone_icepack == nil then
                    mone_icepack = c;
                end
                if c.prefab == "mie_book_silviculture" and mie_book_silviculture == nil then
                    mie_book_silviculture = c;
                end
                if c:HasTag("structure") and structure == nil then
                    structure = c;
                end
            end
            ...
            if stewer and mone_spicepack then
                if exclude_containers and type(exclude_containers) == "table" then
                    exclude_containers[mone_spicepack] = true;
                end
            end

            if stewer and mone_icepack then
                if exclude_containers and type(exclude_containers) == "table" then
                    exclude_containers[mone_icepack] = true;
                end
            end

            if mie_book_silviculture and structure then
                if exclude_containers and type(exclude_containers) == "table" then
                    exclude_containers[mie_book_silviculture] = true;
                end
            end
        end
            ...
            if character ~= nil and inventory ~= nil and container_item ~= nil then
                ...
                for c, bool in pairs(opencontainers) do
                    ...
                    elseif c.prefab == "mone_backpack" then
                        mone_backpack = c;
                    elseif c:HasTag("backpack") then
                        backpack = c;
                    elseif c.prefab == "mone_tool_bag" then
                        mone_tool_bag = c;
                    elseif c.prefab == "mie_book_silviculture" then
                        mie_book_silviculture = c;
                    elseif c:HasTag("structure") then
                        structure = c;
                    end
                end
                ...
                -- 这是干啥的？
                --if mone_tool_bag and mone_backpack then
                --    if exclude_containers and type(exclude_containers) == "table" then
                ...
                -- 背包未打开状态才会生效？话说我这是干嘛的？
                -- 2023-03-06-01:26：这不是多余吗。。。
                --if mone_storage_bag and mone_backpack and not backpack then
                --    if exclude_containers and type(exclude_containers) == "table" then
                --        exclude_containers[mone_backpack] = true;
                --    end
                --end

                -- mie_book_silviculture 和 建筑类容器同时打开的时候，物品先进入建筑类容器
                if mie_book_silviculture and structure then
                    if exclude_containers and type(exclude_containers) == "table" then
                        exclude_containers[mie_book_silviculture] = true;
                    end
