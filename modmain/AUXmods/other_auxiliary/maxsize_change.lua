...
-- 修改堆叠上限。2025/05/25: 新增判定，设置为二者最大值，避免反向堆叠上限
TUNING.STACK_SIZE_MEDITEM = math.max(MAXSIZE_CHANGE, TUNING.STACK_SIZE_MEDITEM);
TUNING.STACK_SIZE_SMALLITEM = math.max(MAXSIZE_CHANGE, TUNING.STACK_SIZE_SMALLITEM);
TUNING.STACK_SIZE_LARGEITEM = math.max(MAXSIZE_CHANGE, TUNING.STACK_SIZE_LARGEITEM);
TUNING.STACK_SIZE_TINYITEM = math.max(MAXSIZE_CHANGE, TUNING.STACK_SIZE_TINYITEM);
-- 2025/05/25: 发现新增了 STACK_SIZE_PELLET
TUNING.STACK_SIZE_PELLET = math.max(MAXSIZE_CHANGE, TUNING.STACK_SIZE_PELLET);
...
env.AddPrefabPostInitAny(function(inst)
    ...
    if inst.components.stackable then
        -- 如果出现不存在 STACK_SIZES 中的值，则设置为 STACK_SIZES 中的最大值
        if not table.contains(STACK_SIZES, inst.components.stackable.maxsize) then
            inst.components.stackable.maxsize = math.max(unpack(STACK_SIZES));
        end
    end
end)
