...
--食人花宝箱
STRINGS[upper("rmi_lureplant")] = L
        and [[
        - 25 格容器
        - 收集所有食人花眼球吞噬的可堆叠的物品，存不下停止
        - 仅在食人花眼球将物品转移至食人花的过程中进行收集
        ]]
        or [[]];

--升级版·猪鼻铸币
STRINGS[upper("rmi_pig_coin")] = L
        and [[
        - 原版的猪鼻铸币的效果，但是延长了召唤时间至 4 分钟
        - 上下地洞会消失
        ]]
        or [[]];

--升级版·挖洞兔号角
STRINGS[upper("mone_rabbitkinghorn")] = L
        and [[
        - 等同于挖洞兔号角，但是无耐久
        ]]
        or [[]];
...
STRINGS[upper("mone_beef_bell")] = L
        and [[
        - 不乱跑 | 防击飞 | 只能被强制攻击
        - 不会生成便便 | 不触发陷阱 | 有训诫值不会发情
        - 不受群体仇恨影响 | 驯服度不会自然下降
        - 重生护符复活牛 | 戴牛角帽无视顺从值直接上牛且回满顺从值
        - 骑牛可以使用武器（此处认定的武器伤害必须大于等于 34，否则不视为武器）
        ]]
        or [[]];
...
--STRINGS[upper("mone_spear_poison")] = L
--        and [[
--        - [2025-11-05] 真实伤害尚未完整实现，测试也不全面
--        - 可以对目标周围造成1.5格范围真实伤害
--        - 注意，真实伤害这个功能可能不够全面，理论上真实伤害应该是对任何目标都能造成伤害...
--        ]]
--        or [[]];
-- 保鲜袋
...
STRINGS[upper("mone_dummytarget")] = L
        and [[
        - 每2秒嘲讽一次半径4格内非友军生物。受到攻击会反伤，反伤值为50点。
        - 击杀傀儡者，会受到10%其最大生命值的真实伤害。
        - 被动：青蛙/蜘蛛女王必须死。注意：玩家使用锤子攻击是可以杀死傀儡的。
