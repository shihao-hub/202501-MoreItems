...
if not config_data.mone_piggbag_itemtestfn then
    ...
    prefabs_table = {
        ...
        "mone_redlantern",
        --"hat_lichen",
        "mie_book_horticulture", -- 2025/06/03
    }
end
table.insert(prefabs_table, "mie_storage_ring");
table.insert(prefabs_table, "那八的空间戒指");

if not config_data.piggybag__only_hold_container then
    ...
    -- 只能存放指定类型的容器（目前为无法被装备的容器）
    env.AddPrefabPostInitAny(function(inst)
        if not TheWorld.ismastersim then
            return
        end
        
        local is_special_container = inst.components.container and not inst.components.equippable
        if is_special_container then
            inst:AddTag("mone_piggybag_itemtesttag");
        else
