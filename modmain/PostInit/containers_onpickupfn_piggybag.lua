...
if config_data.containers_onpickupfn_piggybag then
    ...
    function fns.isDesignatedContainer(item)
        ...
        return not tool_bag_auto_open and item.prefab == "mone_tool_bag"
                ...
                or not icepack_auto_open and item.prefab == "mone_icepack"
                or not spicepack_auto_open and item.prefab == "mone_spicepack"
                or item.prefab == "mie_book_silviculture"
                or item.prefab == "mone_wathgrithr_box"
                or item.prefab == "medal_box";
    end
    local vars = {};
    vars.prefabs = { "mie_book_silviculture", "mone_wathgrithr_box" };
    if not tool_bag_auto_open then
        table.insert(vars.prefabs, "mone_tool_bag");
    end
    if not icepack_auto_open then
        table.insert(vars.prefabs, "mone_icepack");
    end
    if not spicepack_auto_open then
        table.insert(vars.prefabs, "mone_spicepack");
    end
