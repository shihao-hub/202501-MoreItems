...
local log = require("moreitems.main").shihao.base.log
local inspect = require("moreitems.main").thirdparty.inspect
...
local function validfn(inst)
    return inst.replica.container ~= nil and not inst.replica.container:IsEmpty()
            and not inst.replica.container:IsInfiniteStackSize() -- 如果被升级了，则按钮失效
end
    ...
    local function PickUpItems(inst)
        ...
        for _, v in ipairs(ents) do
            if isValid(v) and v.components.inventoryitem
                ...
                "mie_ordinary_bundle_state1", "mie_ordinary_bundle_state2",
            }, v.prefab) then
...
GLOBAL.moreitems_StorageButton = StorageButton
...
function DoFns.lureplant()
    ...
    for y = 2, -2, -1 do
        for x = -2, 2 do
            table.insert(params.rmi_lureplant.widget.slotpos, Vector3(75 * x, 75 * y, 0))
        end
    end
...
function DoFns.chiminea()
    ...
    local common_chest = {
        ...
        itemtestfn = function(container, item, slot)
            -- return not (item:HasTag("irreplaceable") or item:HasTag("_container") or item:HasTag("bundle") or item:HasTag("nobundling"))
            return item:HasTag("_perishable_mone") or item:HasTag("fertilizerresearchable")
        end
    }

    -- 25/08/03：重置，去掉按钮
    common_chest.widget.buttoninfo = nil
    common_chest.widget.buttoninfo = {
        text = "一键整理",
        position = Vector3(0, -75 * 3 + 10, 0),
        fn = fn,
        validfn = validfn
    ...
    params.mone_wardrobe = {
        ...
        itemtestfn = function(container, item, slot)
            ...
            return item:HasTag("_equippable")
                    ...
                    or string.find(item.prefab, "^saddle")
                    or item:HasTag("fan")
                    or string.match(item.prefab, "wx78module_") ~= nil
                    or string.find(item.prefab, "^winona_.*_item$")
        end
    }
...
function DoFns.tool_bag()
    params.mone_tool_bag = {
        ...
        type = "mone_tool_bag",
        itemtestfn = function(container, item, slot)
            if item:HasTag("tool_bag_notag") or item:HasTag("_perishable_mone") then
                return false;
            end
            if item:HasTag("tool")
...
-- 小食物袋
function DoFns.spicepack()
    params.mone_spicepack = {
        widget = {
            slotpos = {},
            animbank = "ui_icepack_2x3",
            animbuild = "ui_icepack_2x3",
            pos = Vector3(-300 + 40 + 80, 200 - 40 - 220, 0),
            side_align_tip = 160,
            more_items_buttoninfo = {
                text = emoji.palm,
                text_position = Vector3(0, 1, 0),
                position = Vector3(-120, -141, 0),
                fn = fn,
                validfn = validfn,
            },
        },
    ...
    for y = 0, 2 do
        table.insert(params.mone_spicepack.widget.slotpos, Vector3(-162, -75 * y + 75, 0))
        table.insert(params.mone_spicepack.widget.slotpos, Vector3(-162 + 75, -75 * y + 75, 0))
    end
end
...
function DoFns.icepack()
    params.mone_icepack = {
        ...
        type = "mone_icepack",
        itemtestfn = function(container, item, slot)
            -- or item:HasTag("non_preparedfood");
            -- Q: 25/05/31，我当时为什么加这个？
            return item:HasTag("_perishable_mone")
        end
    }
    ...
    elseif config_data.mone_icepack_capacity == 0 then
        params.mone_icepack.widget.slotpos = {};
        params.mone_icepack.widget.animbank = "ui_icepack_2x3";
        params.mone_icepack.widget.animbuild = "ui_icepack_2x3";
        params.mone_icepack.widget.pos = Vector3(-300 + 40 + 80, 200 - 40 - 220, 0);

        if params.mone_icepack.widget.buttoninfo then
            params.mone_icepack.widget.buttoninfo.position = Vector3(-2, -177, 0);
        elseif params.mone_icepack.widget.more_items_buttoninfo then
            params.mone_icepack.widget.more_items_buttoninfo.position = Vector3(-120, -141, 0);
        end

        for y = 0, 2 do
            table.insert(params.mone_icepack.widget.slotpos, Vector3(-162, -75 * y + 75, 0))
            table.insert(params.mone_icepack.widget.slotpos, Vector3(-162 + 75, -75 * y + 75, 0))
        end
    end
end
...
-- 打个补丁，兼容一下老麦
local function compatible_with_maxwell0()
    local CONTAINERS = {
        -- 装备袋
...
local function compatible_with_maxwell()
    -- 思路：修改 screens/playerhud.lua 中 OpenSpellWheel 的 upvalue CloseAllChestContainerWidgets
    ...
    local function new_fn(self, ...)
        -- todo: [risky] 此处使用了覆写法
        for k, v in pairs(self.controls.containers) do
            --cheap check for "chest" type containers
            if not table.contains(portable_containers, k.prefab)
                    and v:GetParent() == self.controls.containerroot then
                v:Close()
            end
        end
    end
    UpvalueUtil.SetUpvalue(playerhud.OpenSpellWheel, "CloseAllChestContainerWidgets", new_fn)
end
...
local function moreitems_expansion()
    ...
    params.mie_watersource = {
        ...
        itemtestfn = function(container, item, slot)
            ...
            if item.prefab == "shovel"
                    ...
                    or item.prefab == "mone_farm_plow_item"
            then
                return true;
            end
    ...
    local function get_button_pos_of_mie_fish_box()
        X = 11
        Y = 4
        posX = 98
        posY = 42
    ...
    params.mie_fish_box = {
        widget = {
            ...
            more_items_storage_buttoninfo = {
                text = "高级整理",
                position = get_button_pos_of_mie_fish_box() + Vector3(0, 423 + 1, 0),
                fn = storage_fn,
                validfn = storage_validfn
            }
        },
